"""
PhishGuard AI - Backend API (Windows-compatible fixed version)
"""

import os, sys, re, json, math, time, hashlib, logging
...
def analyze_details(url: str, features: dict, score: float) -> dict:
    ...
    tld      = ('.' + parts[-1]) if len(parts) > 1 else ''
    red, green = [], []
    if features.get('is_https'):            green.append('Uses HTTPS encryption')
    else:                                   red.append('No HTTPS — connection is unencrypted')
    if features.get('has_ip_address'):      red.append('URL uses a raw IP address instead of a domain — very suspicious')
    if features.get('is_suspicious_tld'):   red.append(f'Free/abused domain extension ({tld}) commonly used for phishing')
    if features.get('is_trusted_tld') and not features.get('is_suspicious_tld'): green.append('Standard trusted domain extension')
    kw = features.get('suspicious_keyword_count', 0)
    if kw >= 3:   red.append(f'Contains {kw} suspicious keywords (login, verify, secure…)')
    elif kw >= 1: green.append(f'Only {kw} sensitive keyword(s) found in URL')
    if features.get('brand_impersonation'): red.append('Appears to impersonate a well-known brand')
    if features.get('url_length', 0) > 100: red.append(f'Very long URL ({features["url_length"]} chars) used to hide real destination')
    else: green.append('URL length looks normal')
    if features.get('subdomain_count', 0) > 3: red.append(f'Excessive subdomain nesting ({features["subdomain_count"]} levels)')
    elif features.get('subdomain_count', 0) == 0: green.append('Clean domain with no suspicious subdomains')
    ...
    else:               verdict = 'This URL appears safe and legitimate. No signs of phishing detected.'
    return {'verdict': verdict, 'red_flags': red, 'green_flags': green, 'domain': hostname, 'protocol': parsed.scheme, 'path': parsed.path, 'has_query': bool(parsed.query)}

def fetch_screenshot(url: str) -> dict:
    if not SCREENSHOT_API_KEY:
        return {'available': False, 'image_url': None, 'error': 'No API key set. Add SCREENSHOT_API_KEY to your .env file.', 'setup_url': 'https://screenshotone.com'}
    try:
        params    = {'access_key': SCREENSHOT_API_KEY, 'url': url, 'viewport_width': 1280, 'viewport_height': 800, 'format': 'jpg', 'image_quality': 80, 'block_ads': 'true', 'block_cookie_banners': 'true', 'timeout': 15, 'delay': 1}
        ...
        return {'available': False, 'image_url': None, 'error': f'Screenshot API returned {r.status_code}'}
    except Exception as e:
...
def get_domain_age(url: str) -> dict:
    ...
    if ml_model:
        ...
        risk_score = min(1.0, features.get('suspicious_keyword_count',0)*0.12 + features.get('is_suspicious_tld',0)*0.30 + (1-features.get('is_https',0))*0.22 + features.get('has_ip_address',0)*0.35 + features.get('brand_impersonation',0)*0.25)
    ...
    result = {
        ...
        'risk_score': round(risk_score*100, 1), 'risk_raw': risk_score,
...
def get_stats():
    ...
    return {'total_scans': total, 'phishing_detected': phishing, 'safe_sites': total-phishing, 'phishing_rate': round(phishing/total*100, 1), 'avg_risk_score': round(sum(s.get('risk_score',0) for s in scan_history)/total, 1), 'risk_distribution': dist}
